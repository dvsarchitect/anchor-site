...
MUST_BUILD_REGEX='^(content/|layouts/|static/|config\.toml|netlify\.toml|hugo\.toml|package\.json)$'
...
for f in $CHANGED; do
  ...
  # anything outside documentation/, README, or .md should force build
  if [[ ! $f =~ ^documentation/ ]] && [[ ! $f =~ README\.md$ ]] && [[ ! $f =~ \.md$ ]]; then
    ONLY_DOCS=false
    break
  fi
done
